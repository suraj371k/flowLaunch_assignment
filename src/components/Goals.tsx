...
const Goals = () => {
  ...
  const getLevelColor = (level) => {
    switch (level) {
      case 'expert': return 'from-purple-500 to-violet-500';
      case 'advanced': return 'from-blue-500 to-cyan-500';
      case 'intermediate': return 'from-green-500 to-teal-500';
  ...
  const GoalItem = ({ goal, type, index }) => (
    ...
    >
      <div className={`
        relative backdrop-blur-sm bg-white/5 rounded-2xl border border-white/10 p-6
        transition-all duration-500 hover:bg-white/10 hover:scale-105 hover:shadow-xl
        ${hoveredGoal === goal.id ? 'shadow-2xl shadow-blue-500/20' : ''}
      `}>
        {/* Animated border glow */}
        <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${type === 'professional' ? getPriorityColor(goal.priority) : getLevelColor(goal.level)} opacity-0 transition-opacity duration-500 ${hoveredGoal === goal.id ? 'opacity-20' : ''} blur-sm -z-10`}></div>
        
        <div className="flex items-start gap-4">
          {/* Icon */}
          <div className={`p-3 rounded-xl bg-gradient-to-r ${type === 'professional' ? getPriorityColor(goal.priority) : getLevelColor(goal.level)} shadow-lg group-hover:scale-110 transition-transform duration-300`}>
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-3">
              ...
              {type === 'personal' && (
                <div className={`px-2 py-1 rounded-full text-xs font-semibold ${
                  goal.level === 'expert' ? 'bg-purple-500/20 text-purple-300' :
                  goal.level === 'advanced' ? 'bg-blue-500/20 text-blue-300' :
                  goal.level === 'intermediate' ? 'bg-green-500/20 text-green-300' :
            ...
            {/* Progress bar for professional goals */}
            {type === 'professional' && (
              <div className="space-y-2">
                ...
                <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                  <div 
                    className={`h-full bg-gradient-to-r ${getPriorityColor(goal.priority)} rounded-full transition-all duration-1000 ease-out`}
                    style={{
                      width: animationTrigger ? `${goal.progress}%` : '0%',
            ...
            {/* Interest level for personal interests */}
            {type === 'personal' && (
                  ...
                  {[...Array(5)].map((_, i) => (
                    <div
                      ...
                      className={`w-2 h-2 rounded-full transition-all duration-300 ${
                        i < (goal.level === 'expert' ? 5 : goal.level === 'advanced' ? 4 : 3)
            ...
            <div className="mt-8 p-6 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-white font-bold">
                  {Math.round(professionalGoals.reduce((acc, goal) => acc + goal.progress, 0) / professionalGoals.length)}%
                </span>
              </div>
              <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{
                    width: animationTrigger ? `${Math.round(professionalGoals.reduce((acc, goal) => acc + goal.progress, 0) / professionalGoals.length)}%` : '0%'
                  }}
                ></div>
              </div>
            </div>
          ...
          <div className="space-y-8">
            ...
            <div className="mt-8 grid grid-cols-2 gap-4">
              {['Creative', 'Learning', 'Outdoor', 'Technical'].map((category, i) => (
                <div key={category} className="p-4 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 text-center hover:bg-white/10 transition-colors duration-300">
